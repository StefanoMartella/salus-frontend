 ...
 * -------------------------
 *
 * Passaggi per ottenere questi file:
 * - runna il tuo programma, in sto caso Salus, vai sulla porta dove runna, in sto caso localhost:8080/v3/api-docs.yaml, e ti si genererà e scaricherà il file yaml del tuo progetto
 * - editor.swagger.io: File --> import file --> e carica il file.yaml appena scaricato. Ti si genererà il codice sulla sinistra, poi: Generate Client --> typescript-axios e ti si scaricherà uno zip con tutti i file riguardanti il tuo progetto.
 * - Estrai lo zip e dentro il tuo progetto di front end crea una cartella "api" e dentro mettici alcune cose prese dalla cartella estratta:
 *      - cartella api
 *      - cartella models
 *      - files: api.ts, base.ts, configuration.ts, index.ts
 *
 * Questi file sono parti di un modulo di client API per fare richieste HTTP al backend tramite Axios.
 */
/*import:
    - globalAxios serve per le richieste HTTP con i metodi HTTP
    - AxiosResponse rappresenta la struttura della risposta ricevuta da una richiesta Axios. Contiene proprietà come data, status, headers...
    - AxiosInstance rappresenta un'istnaza di Axios
    - AxiosRequestConfig: Questo è un tipo che rappresenta la configurazione per una richiesta Axios. È un oggetto che contiene varie opzioni e proprietà per personalizzare una richiesta HTTP, come gli headers, i parametri, il metodo HTTP, l'autenticazione, ecc.
*/
import globalAxios, {
  AxiosResponse,
  AxiosInstance,
...
//funzione che restituisce un oggetto con diverse funzioni che rappresentano i metodi HTTP che abbiamo definito noi sul backend: find, count, delete...
//prende in input configuration, che è un file con un'interfaccia che abbiamo preso dalla cartella estratta: indica le configurazione di base per le chiamate con axios: chiave di sicurezza, password, username, basePath..
export const DipendenteControllerApiAxiosParamCreator = function (
  ...
  return {
    ...
    count: async (
      ...
      //dato che ci restituiva interamente un oggetto invece che i risultati, abbiamo dovuto flattare la criteria e commentare l'istruzione if sotto
    ...
    create2: async (
      ...
      const localVarPath = `/api/dipendente`; //percorso relativo dove verrà effettuata la richiesta HTTP. Sotto creiamo invece un URL assoluto
      // use dummy base URL string because the URL constructor only accepts absolute URLs.
      const localVarUrlObj = new URL(localVarPath, "https://example.com");
      let baseOptions;
      if (configuration) {
      ...
      // Scorre tutte le chiavi dell'oggetto localVarQueryParameter e imposta i loro valori corrispondenti nell'oggetto query. In questo caso, l'oggetto localVarQueryParameter è vuoto, quindi questa parte del codice non aggiunge alcun parametro di ricerca all'URL.
      for (const key in localVarQueryParameter) {
        query.set(key, localVarQueryParameter[key]);
      }
      for (const key in options.params) {
...
//qui sotto abbiamo praticamente la stessa funzione, ma scritta con programmazione funzionale
/**
